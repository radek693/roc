    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            while (true) // cyklus, který se opakuje dokud je podmínka prohra false
            {
                ...
                if (input.Length != 2 || !int.TryParse(input[0], out startovniradek) || !int.TryParse(input[1], out startovnisloupec))
                ...
                if (input2.Length != 2 || !int.TryParse(input2[0], out konecnyradek) || !int.TryParse(input2[1], out konecnysloupec))
                {
                    Console.WriteLine("Neplatný Tah (Pro pokračování stiskněte enter)");
                    Console.ReadKey();
                    continue;
